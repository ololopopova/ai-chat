"""Products Agent Tool — эксперт по БАДам и биохакингу."""

from __future__ import annotations

from langchain_core.tools import tool

from src.core.logging import get_logger

logger = get_logger(__name__)


@tool
async def products_agent(query: str) -> str:
    """
    Агент по продуктам: БАДы, микродозинг грибов, биохакинг, рецепты и привычки.

    Вызывай этот инструмент когда пользователь спрашивает про:
    - Конкретные БАДы и добавки
    - Дозировки и схемы приёма
    - Микродозинг грибов
    - Курс биохакинга
    - Рецепты и здоровые привычки
    - Рекомендации по продуктам для конкретных целей

    Args:
        query: Вопрос пользователя или уточненный запрос

    Returns:
        Информация о продуктах от специалиста
    """
    logger.info(
        "Products agent called",
        extra={"query_preview": query[:100] if query else "empty"},
    )

    # TODO: Phase 6 - здесь будет реальный MCP-агент с RAG
    # Пока возвращаем заглушку с реалистичными данными
    stub_response = """**Рекомендации для улучшения сна:**

🌙 **Мелатонин (3-5 мг)**
- Регулирует циркадные ритмы
- Принимать за 30-60 минут до сна
- Не вызывает привыкания

🍵 **L-теанин (200-400 мг)**
- Способствует расслаблению без сонливости
- Хорошо работает в комбинации с магнием
- Можно принимать вечером

✨ **Магний глицинат (400 мг)**
- Успокаивает нервную систему
- Улучшает качество сна
- Лучшая форма для усвоения

⚠️ Начинайте с минимальных дозировок и постепенно увеличивайте при необходимости."""

    logger.info("Products agent completed")
    return stub_response

