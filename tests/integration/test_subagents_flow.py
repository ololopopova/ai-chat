...
async def test_full_flow_compatibility_query(chat_graph) -> None:
    ...
    assert any(
        keyword in content
        for keyword in ["мелатонин", "магний", "совместим", "сочета", "комбинац", "синерг"]
    )
...
async def test_full_flow_multiple_tools(chat_graph) -> None:
    ...
    has_product_info = any(
        keyword in content for keyword in ["мелатонин", "магний", "теанин", "дозир"]
    )
    has_compatibility_info = any(
        keyword in content for keyword in ["совместим", "сочета", "комбинац", "синерг", "вместе"]
    )
...
async def test_full_flow_with_history(chat_graph) -> None:
    ...
    assert any(keyword in content for keyword in ["мг", "доз", "прин", "рекоменд"])
...
async def test_full_flow_empty_query(chat_graph) -> None:
    ...
    result = await chat_graph.ainvoke({"messages": [HumanMessage(content="")]})
